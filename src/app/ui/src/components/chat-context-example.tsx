			...
			<CardContent className='space-y-3'>
				...
				<div className='flex space-x-2'>
					<Input
						...
						className='text-xs'
					/>
					<Button size='sm' onClick={handleSwitchSession} disabled={!newSessionId.trim()}>
						Switch
					</Button>
				</div>

				<Button size='sm' variant='outline' onClick={returnToWelcome} disabled={isWelcomeState}>
					Return to Welcome
				</Button>
			</CardContent>
		</Card>
		...
		<Card className='flex-1'>
			<CardHeader>
				<div className='flex items-center justify-between'>
					<CardTitle className='text-sm'>Messages ({messageCount})</CardTitle>
					<Button size='sm' variant='outline' onClick={reset} disabled={!hasMessages}>
						Reset
					</Button>
				</div>
			</CardHeader>
											...
											<div>
												...
												{message.toolResult && (
													<div className='mt-2 text-xs'>
														...
														<pre className='opacity-70'>{formatToolResult(message.toolResult)}</pre>
													</div>
												)}
											</div>
										) : (
