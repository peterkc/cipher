...
import { ExportStates, MessageData, ModalStates, OperationStates, QuickAction } from '@/types/chat';
...
interface ChatProps {
	...
	messages?: Message[];
	sendMessage?: (content: string, imageData?: MessageData, fileData?: MessageData) => Promise<void>;
...
export function Chat({
	...
	const handleSend = useCallback(
		async (content: string, imageData?: MessageData, fileData?: MessageData) => {
	...
	const quickActions: QuickAction[] = [
		...
		},
		{
			...
			action: () =>
				handleSend(
					"Help me store an important programming concept, design pattern, or coding technique that I can reference later. Please ask me what concept I'd like to store and then save it with proper examples and explanations."
				),
			icon: '💡',
		},
		{
		...
		},
		{
			...
			action: () =>
				handleSend(
					'Show me how to use one of your available tools. Pick an interesting one and demonstrate it.'
				),
			icon: '⚡',
		},
	];
			...
			<main className='flex-1 flex flex-col relative'>
				...
				<div className='flex-1 flex overflow-hidden'>
					<div className='flex-1 flex flex-col'>
						...
						<InputArea onSend={handleSend} disabled={isSendingMessage} />
					</div>
				...
				<ErrorNotification message={errorMessage} onDismiss={() => setErrorMessage(null)} />
			</main>
		</div>
	);
}
