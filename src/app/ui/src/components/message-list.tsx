...
import { ContentPart, Message } from '@/types/server-registry';
...
export function MessageList({ messages, className, maxHeight = 'h-full' }: MessageListProps) {
	...
	function isValidDataUri(src: string): boolean {
		const dataUriRegex =
			/^data:image\/(png|jpeg|jpg|gif|webp|svg\+xml);base64,[A-Za-z0-9+/]+={0,2}$/i;
	...
	const renderImagePart = (part: any, index: number) => {
		const src =
			part.base64 && part.mimeType ? `data:${part.mimeType};base64,${part.base64}` : part.base64;
	...
	const renderTextPart = (part: any, index: number) => {
		return (
			<pre
				key={index}
				className='whitespace-pre-wrap overflow-auto bg-background/50 p-2 rounded text-xs text-muted-foreground my-1'
			>
				{part.text}
			</pre>
		);
	...
	const renderUnknownPart = (part: any, index: number) => {
		return (
			<pre
				key={index}
				className='whitespace-pre-wrap overflow-auto bg-muted/50 p-2 rounded text-xs my-1'
			>
	...
	const getToolStatusIcon = (msg: Message, allMessages: Message[]) => {
		...
		if (msg.role === 'system' && msg.toolExecutionId) {
			const hasResult = allMessages.some(
				m =>
					m.role === 'system' &&
	...
	const getRoleIcon = (role: string) => {
		switch (role) {
			...
			case 'assistant':
				return (
					<Image src='/cipher-logo.svg' alt='Cipher' width={16} height={16} className='w-4 h-4' />
				);
			case 'system':
			...
			if (msg.role === 'tool') {
				return (
					<div className='text-sm text-muted-foreground'>
						...
						{msg.toolResult && <div className='text-xs'>Click to view arguments and results</div>}
					</div>
				);
			}
			return <div></div>;
		...
		<ScrollArea className={cn(maxHeight, className, 'flex-1 scrollbar-thin')} ref={scrollAreaRef}>
			<div className='space-y-1 p-4'>
				{messages
					...
					.map((msg, idx) => {
						const { isUser, isAi, isSystem, isLastMessage, isToolRelated, isSystemToolMessage } =
						...
						return (
							...
							>
								...
								{!isSystem || isToolProgress || isSystemToolMessage ? (
									<Avatar className='w-8 h-8 shrink-0'>
										<AvatarFallback className='text-xs'>
											{isToolProgress || isSystemToolMessage ? (
								...
								<div
									className={
										msg.role === 'tool' || isSystemToolMessage
								...
								>
									...
									>
										...
										{(msg.role === 'tool' ||
											(isToolRelated && isToolPanelExpanded(msg.id)) ||
											!isToolRelated) && (
											<>
												{/* Main content */}
													...
													<div className='mt-3 space-y-2 border-t pt-2'>
														...
														{msg.role === 'tool' && (
															<div>
																...
																{msg.toolArgs &&
																typeof msg.toolArgs === 'object' &&
																Object.keys(msg.toolArgs).length > 0 ? (
																	<pre className='whitespace-pre-wrap overflow-auto bg-muted/50 p-2 rounded text-xs'>
										...
										{!isToolPanelExpanded(msg.id) &&
											(isToolRelated || isSystemToolMessage) &&
											msg.role !== 'tool' && (
				...
				{/* Thinking indicator */}
				{hasThinkingMessage && <ThinkingIndicator className='mb-4' />}
