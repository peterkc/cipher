...
export function ServerConfigModal({
	...
	const updateField = (key: string, value: string, isEnv: boolean = false) => {
		if (isEnv) {
			setEnvVars(prev => prev.map(field => (field.key === key ? { ...field, value } : field)));
		} else {
			setConfigFields(prev => prev.map(field => (field.key === key ? { ...field, value } : field)));
		}
	};
						...
						<div className='flex items-start gap-3 p-3 bg-blue-50 border border-blue-200 rounded-lg'>
							...
							<div className='text-sm'>
								...
								<p className='text-blue-700'>
									This server requires additional configuration before installation.
								</p>
							</div>
						</div>
					) : (
						<div className='flex items-start gap-3 p-3 bg-green-50 border border-green-200 rounded-lg'>
							...
							<div className='text-sm'>
								...
								<p className='text-green-700'>
									This server is ready to install with default configuration.
								</p>
							</div>
						</div>
					)}
							...
							{configFields.map(field => (
								<div key={field.key} className='space-y-2'>
									...
									<Input
										...
										className={cn(field.required && !field.value && 'border-red-300')}
									/>
