...
export function ServerRegistryBrowser({
	...
	const [selectedServerForConfig, setSelectedServerForConfig] =
		React.useState<ServerRegistryEntry | null>(null);
	...
	const needsConfiguration = (entry: ServerRegistryEntry): boolean => {
		...
		if (entry.config.type === 'stdio') {
			...
			const hasPlaceholders = entry.config.args?.some(
				arg =>
			...
			const hasEmptyEnv =
				entry.config.env && Object.values(entry.config.env).some(val => val === '');
		...
		if (entry.config.type === 'sse' || entry.config.type === 'streamable-http') {
			...
			const hasPlaceholderHeaders =
				entry.config.headers &&
				Object.values(entry.config.headers).some(
					val => val.includes('your-') || val.includes('Bearer token') || val === ''
				);
		...
		<div className='p-4 border rounded-lg bg-card hover:bg-muted/50 transition-colors'>
			<div className='flex items-start justify-between mb-3'>
				<div className='flex items-center gap-3 min-w-0 flex-1'>
					{entry.icon && <span className='text-2xl flex-shrink-0'>{entry.icon}</span>}
					<div className='min-w-0 flex-1'>
						...
						<div className='flex items-center gap-4 text-xs text-muted-foreground'>
							...
							{entry.version && (
								<span className='flex items-center gap-1'>
									<Package className='w-3 h-3' />v{entry.version}
								</span>
							)}
							<span className='flex items-center gap-1'>
			...
			<div className='flex items-center justify-between text-xs'>
				...
				<span className='text-muted-foreground uppercase'>{entry.config.type}</span>
			</div>
		</div>
	);
			...
			<DialogContent className='max-w-4xl max-h-[90vh] flex flex-col'>
				...
				<div className='space-y-4 border-b pb-4'>
					<div className='flex gap-2'>
						...
						<Button variant='outline' size='sm' onClick={refreshEntries} disabled={isLoading}>
				...
				<div className='flex items-center justify-between pt-4 border-t text-sm text-muted-foreground'>
					...
					<span>{entries.filter(e => e.isInstalled).length} installed</span>
				</div>
			</DialogContent>
