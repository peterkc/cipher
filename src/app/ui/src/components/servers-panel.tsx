...
export function ServersPanel({
			...
			try {
				...
				if (!response.ok) {
					const errorData = await response
						.json()
						.catch(() => ({ error: 'Failed to fetch servers' }));
					throw new Error(
						errorData.message || errorData.error || `Server List: ${response.statusText}`
					);
				}
	...
	const handleServerSelect = React.useCallback(
			...
			try {
				...
				if (!response.ok) {
					...
					throw new Error(
						errorData.message ||
							errorData.error ||
							`Tool List (${server.name}): ${response.statusText}`
					);
				}
				...
				console.log(
					`🔨 Extracted ${tools.length} tools for ${serverId}:`,
	...
	const handleDeleteServer = async (serverId: string) => {
		...
		try {
			...
			if (!response.ok) {
				...
				throw new Error(
					errorData.message || errorData.error || `Server Removal: ${response.statusText}`
				);
			}
			...
			window.dispatchEvent(
				new CustomEvent('mcp-server-uninstalled', {
	...
	const ServerStatusIndicator = ({ status }: { status: string }) => (
		<div className={cn('w-2 h-2 rounded-full flex-shrink-0', getServerStatusColor(status))} />
	);
