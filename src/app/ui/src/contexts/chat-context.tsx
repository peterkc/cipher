...
import { useCreateSession, useSessionOperations, useSessionSwitch } from '@/hooks/use-sessions';
import {
...
export function ChatProvider({
	...
	const { currentSessionId, isWelcomeState, enableOptimizations, getCachedSession } =
		useSessionStore();
	...
	const sendMessage = useCallback(
		async (content: string, imageData?: { base64: string; mimeType: string }) => {
		...
		[originalSendMessage, currentSessionId, isWelcomeState, isStreaming, ensureDefaultSession]
	);

	// Quick action send message - always uses "default" session
	const sendQuickActionMessage = useCallback(
		...
		if (typeof window !== 'undefined') {
			...
			return () => {
				...
				window.removeEventListener(
					'cipher:switchSession',
					handleExternalSessionSwitch as EventListener
				);
